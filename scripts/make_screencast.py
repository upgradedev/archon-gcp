#!/usr/bin/env python3
"""Render a captured Archon run as a 1920x1080 terminal screencast (silent).

Archon is a CLI/ADK agent, so the "screencast" is the agent RUNNING. CI renders a
CAPTURED real run: docs/screencast_transcript.txt holds the actual output of
`python -m archon.cli` (deterministic books + R1-R4 eval gate) followed by
`python scripts/demo_sequential.py` (the real Google ADK SequentialAgent driven by
offline scripted models). Each line is prefixed with the second at which it should
appear. We draw a terminal "typewriter" where lines stream in on that timeline, the
view scrolls like a real shell, and the final frame is held to TARGET_SECONDS so the
downstream compose math (title + screencast + outro) is exact.
...
Env / args (all optional):
  TARGET_SECONDS   total screencast length; final frame is held to this (default 160)
...
# Colours by author / line kind.
COL_DEFAULT = (201, 209, 217)
COL_PROMPT = (126, 231, 135)
COL_RULE = (88, 96, 105)
COL_HEADER = (52, 211, 153)     # emerald (ARCHON / SequentialAgent headers)
COL_OK = (126, 231, 135)        # green ticks
COL_FAIL = (248, 113, 113)      # red crosses
COL_DIM = (139, 148, 158)
AUTHOR_COLOURS = {
    "reconciler": (121, 192, 255),   # blue
    "validator": (240, 214, 110),    # yellow
    "narrator": (52, 211, 153),      # emerald
}
...
def line_colour(text: str):
    ...
    if t.startswith("ARCHON") or t.startswith("Google ADK SequentialAgent"):
        return COL_HEADER, True
    if t.startswith("✓"):
        return COL_OK, False
    if t.startswith("✗"):
        return COL_FAIL, False
    m = re.match(r"^\[(\w+)", t)
    if m and m.group(1) in AUTHOR_COLOURS:
        return AUTHOR_COLOURS[m.group(1)], False
...
def main() -> int:
    ...
    target = float(os.environ.get("TARGET_SECONDS", "160"))
    ...
    def render(n_visible: int) -> Image.Image:
        ...
        d.text((150, 18), "Archon  -  autonomous bookkeeper (GCP)", font=font, fill=COL_DIM)
    ...
    tmpdir = tempfile.mkdtemp(prefix="archon_screencast_")
